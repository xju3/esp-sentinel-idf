...
esp_err_t test_lis2dh12_patrol_run(void)
{
    ...
    esp_err_t err = drv_lis2dh12_init();
    // drv_lis2dh12_init is designed to be called multiple times.
    // It returns ESP_OK if already initialized. We only log if it's a real failure.
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "LIS2DH12 initialization failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    {
        ESP_LOGE(TAG, "DAQ Failed with error: %s", esp_err_to_name(err));
    }
    return err;
}
...
{
    TEST_ASSERT_EQUAL(ESP_OK, test_lis2dh12_patrol_run());
}