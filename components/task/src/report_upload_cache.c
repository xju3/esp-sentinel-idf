                                    ...
                                    void *ctx)
{
    ...
    while (true) {
        ...
        // 按巡逻时间(patrol)去计算延迟，损失一些精度但可以容忍重启导致的时间乱序
        uint32_t patrol_s = (g_user_config.patrol > 0 ? g_user_config.patrol : 60) * 60;
        uint32_t delay_s = (uint32_t)index.count * patrol_s;
