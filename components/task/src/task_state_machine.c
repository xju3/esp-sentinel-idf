...
typedef struct
{
    ...
    // RMS values for message
    float rms_x, rms_y, rms_z, rms_mean;
...
static void process_analysis_window(StateAnalysisContext_t *ctx)
{
    ...
    // Store RMS values for message
    ctx->rms_x = std_x;
    ctx->rms_y = std_y;
    ctx->rms_z = std_z;
    ctx->rms_mean = current_rms;

    // 2. Perform FFT on the axis with strongest RMS (stddev magnitude)
    AxisId_t fft_axis = select_fft_axis(std_x, std_y, std_z);
    const float *fft_input = (fft_axis == AXIS_X) ? ctx->fft_input_x : (fft_axis == AXIS_Y) ? ctx->fft_input_y
                                                                                            : ctx->fft_input_z;
...
static void task_state_check_handler(void *pvParameters)
{
    ...
    while (1)
    {
        if (xSemaphoreTake(g_state_check_semaphore, portMAX_DELAY) == pdTRUE)
        {
            ...
            send_machine_status_message(&context);
            unlock_system_task();
        }
    }
}
