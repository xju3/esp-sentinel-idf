...
esp_err_t set_device_baseline(uint32_t duration_ms, const char *device_id)
{
    ...
    esp_err_t err = sys_daq_capture(&cfg, duration_ms, baseline_chunk_handler, &welford_st);
    if (err == ESP_OK)
    {
        // ==============================================================
    ...
    vStreamBufferDelete(task_monitor_imu_stream);
    task_monitor_imu_stream = NULL;
    if (err != ESP_OK)
    {
