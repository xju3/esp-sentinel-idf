...
//     After each INT1 event the HPF baseline auto-resets to current
//     acceleration. Dynamic "delta from current state" detector — ideal for
//     impulse detection.
//
...
static bool s_lis2dh12_initialized = false;
...
static esp_err_t lis2dh12_read_multiple(uint8_t reg, uint8_t *buffer,
                                        size_t len);
...
static float lis2dh12_config_odr_callback(float ideal_odr) {
  ...
  uint8_t ctrl_reg1_val =
      (uint8_t)((selected_reg << 4) | 0x07); // XYZ enabled, normal mode
...
esp_err_t drv_lis2dh12_init(void) {
  ...
  if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
    LOG_ERRORF("drv_lis2dh12_init: spi_bus_initialize failed: %s",
               esp_err_to_name(ret));
    return ret;
  }
  ...
  spi_device_interface_config_t devcfg = {
      .clock_speed_hz =
          1 * 1000 * 1000, // 调试阶段：降速至 1MHz 与 LIS2DH12 保持一致
  ...
  // Perform a dummy read to wake up SPI interface (ST sensors sometimes need
  // this)
  uint8_t dummy;
  ...
  if (who_am_i != LIS2DH12_WHO_AM_I_VAL) {
    LOG_ERRORF("WHO_AM_I check failed. Got 0x%02x, expected 0x%02x", who_am_i,
               LIS2DH12_WHO_AM_I_VAL);
...
lis2dh12_fs_t drv_lis2dh12_get_current_fs(void) { return s_current_fs; }
...
esp_err_t drv_lis2dh12_enable_wom(const lis2dh12_wom_cfg_t *wom_cfg) {
  ...
  //   INT_THS is 7-bit unsigned, step depends on FS; clamp to [WOM_THS_MIN,
  //   WOM_THS_MAX].
  ...
  if (ret != ESP_OK)
    return ret;
  LOG_DEBUGF(
      "THS1=%u (~%u mg req=%u), THS2=%u (~%u mg req=%u) [FS=±%ug, %umg/LSB]",
      thr1, thr1 * ths_step_mg, wom_cfg->threshold_mg_int1, thr2,
      thr2 * ths_step_mg, wom_cfg->threshold_mg_int2, lis2dh12_fs_g(wom_fs),
      ths_step_mg);
  ...
  if (ret != ESP_OK)
    return ret;
  LOG_DEBUGF("Duration INT1=%u ms, INT2=%u ms", wom_cfg->duration_int1 * 20u,
             wom_cfg->duration_int2 * 20u);
  ...
  // This helps avoid repeated interrupts caused by undefined HPF state after
  // reset.
  {
    uint8_t hpf_ref = 0;
  ...
  LOG_DEBUGF("WoM enabled — INT1: >%u mg, INT2: >%u mg", thr1 * ths_step_mg,
             thr2 * ths_step_mg);
  return ESP_OK;
}
