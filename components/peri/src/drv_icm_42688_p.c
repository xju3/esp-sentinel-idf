 ...
 * HAL 硬件抽象层 ODR 搓配回调
 * ========================================================================= */
typedef struct { float odr_hz; uint8_t reg_value; } ICM_ODR_Map_t;

static const ICM_ODR_Map_t icm_odr_table[] = {
    {12.5f, 0x0B}, {25.0f, 0x0A}, {50.0f, 0x09}, {100.0f, 0x08},
    {200.0f, 0x07}, {500.0f, 0x0F}, {1000.0f, 0x06}, {2000.0f, 0x05},
    {4000.0f, 0x04}, {8000.0f, 0x03}, {16000.0f, 0x02}, {32000.0f, 0x01}
};
...
void drv_icm42688_clear_wom_interrupt(void) {
    uint8_t dummy = 0;
    icm_read_reg(0x2D, &dummy); icm_read_reg(0x37, &dummy); icm_read_reg(0x38, &dummy); 
}

esp_err_t enable_icm42688p_wom(uint16_t threshold_mg) {
    icm_write_reg(0x57, 0x00); icm_write_reg(0x65, 0x00); icm_write_reg(0x66, 0x00); 
    icm_write_reg(ICM_REG_FIFO_CONFIG, 0x00); icm_write_reg(0x4B, 0x02); 
    ...
    uint8_t status = 0xFF;
    for (int i = 0; i < 20; i++) {
        icm_read_reg(0x2D, &status); icm_read_reg(0x37, &status);
...
esp_err_t drv_icm42688_init() {
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .spics_io_num = PIN_NUM_CS,         .queue_size = 7,   
        .command_bits = 8, 
    };
    ...
    if (who_am_i != 0x47) return ESP_ERR_NOT_FOUND;
...
static void icm_dma_worker_task(void *arg) {
    ...
    trans[0].length = DMA_CHUNK_BYTES * 8; trans[0].rx_buffer = s_dma_ping; trans[0].cmd = ICM_REG_FIFO_DATA | 0x80;
    trans[1].length = DMA_CHUNK_BYTES * 8; trans[1].rx_buffer = s_dma_pong; trans[1].cmd = ICM_REG_FIFO_DATA | 0x80;
