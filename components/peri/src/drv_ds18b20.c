...
static esp_err_t ds18b20_read_scratchpad(uint8_t *scratchpad)
{
    ...
    if (crc != scratchpad[DS18B20_CRC_INDEX]) {
        LOG_WARNF("DS18B20 scratchpad CRC mismatch: calc=0x%02X recv=0x%02X",
                  crc,
...
esp_err_t drv_ds18b20_init(void)
{
    ...
    if (ret == ESP_OK) {
        s_current_resolution = ds18b20_resolution_from_config(scratchpad[DS18B20_CONFIG_INDEX]);
    } else {
        LOG_WARNF("Failed to read DS18B20 resolution, using 12-bit timing: %s", esp_err_to_name(ret));
...
esp_err_t drv_ds18b20_read_temperature(float *temperature)
{
    ...
    for (int attempt = 0; attempt < DS18B20_READ_RETRIES; ++attempt) {
        ...
        int16_t raw_temp = (int16_t)(((uint16_t)scratchpad[DS18B20_TEMP_MSB_INDEX] << 8) |
...
esp_err_t drv_ds18b20_self_test(void)
{
    ...
    LOG_DEBUGF("DS18B20 scratchpad temp_lsb=0x%02X temp_msb=0x%02X th=0x%02X tl=0x%02X cfg=0x%02X",
               scratchpad[DS18B20_TEMP_LSB_INDEX],
               scratchpad[DS18B20_TEMP_MSB_INDEX],
               scratchpad[DS18B20_TH_INDEX],
               scratchpad[DS18B20_TL_INDEX],
               scratchpad[DS18B20_CONFIG_INDEX]);
    ...
    if (temperature < DS18B20_TEMP_MIN_C || temperature > DS18B20_TEMP_MAX_C) {
        LOG_WARNF("DS18B20 temperature outside absolute range: %.3f C", temperature);
    } else {
