...
static esp_err_t iis3dwb_write_reg_with_freq(uint8_t reg, uint8_t data,
                                             uint32_t freq_hz) {
  spi_transaction_t t = {
      .length = 8,
      .cmd = reg & 0x7F,
...
static esp_err_t iis3dwb_read_bytes_with_freq(uint8_t reg, uint8_t *data,
                                              size_t len, uint32_t freq_hz) {
  if (!data || len == 0U)
...
static esp_err_t iis3dwb_read_reg_with_freq(uint8_t reg, uint8_t *data,
                                            uint32_t freq_hz) {
...
static uint32_t iis3dwb_post_release_settle_ms(void) {
  return (CONFIG_SENTINEL_SENSOR_POWER_SETTLE_MS >
...
static esp_err_t iis3dwb_hold_sdo_low_during_bootstrap(void) {
  ...
  // IIS3DWB_PIN_NUM_SDO, IIS3DWB_SPI_BOOTSTRAP_HOLD_MS);
  ...
  // LOG_INFOF("IIS3DWB bootstrap: release SDO/MISO and wait %lu ms before SPI
  // probe", (unsigned long)settle_ms);
...
static esp_err_t iis3dwb_init_spi_resources(void) {
  ...
  esp_err_t ret =
      spi_bus_initialize(IIS3DWB_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO);
  if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
    LOG_ERRORF("drv_iis3dwb_init: spi_bus_initialize failed: %s",
               esp_err_to_name(ret));
    return ret;
  }
  ...
  if (!s_dma_ping_raw)
    s_dma_ping_raw = (uint8_t *)heap_caps_aligned_alloc(
        32, DMA_RAW_BYTES, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!s_dma_pong_raw)
    s_dma_pong_raw = (uint8_t *)heap_caps_aligned_alloc(
        32, DMA_RAW_BYTES, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!s_dma_ping_raw || !s_dma_pong_raw)
    return ESP_ERR_NO_MEM;

  if (!s_conv_ping) {
    s_conv_ping = (imu_raw_data_t *)heap_caps_malloc(
        sizeof(imu_raw_data_t) * DMA_CHUNK_SAMPLES,
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  }
  if (!s_conv_pong) {
    s_conv_pong = (imu_raw_data_t *)heap_caps_malloc(
        sizeof(imu_raw_data_t) * DMA_CHUNK_SAMPLES,
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  }
...
static esp_err_t iis3dwb_probe_identity(void) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    esp_err_t ret = iis3dwb_read_reg_with_freq(
        IIS3DWB_REG_WHO_AM_I, &who_am_i[i], IIS3DWB_SPI_PROBE_HZ);
    if (ret != ESP_OK)
      return ret;
  }
  ...
  for (size_t i = 0; i < 3; ++i) {
    if (who_am_i[i] != IIS3DWB_WHO_AM_I_VAL) {
      LOG_ERRORF("IIS3DWB WHO_AM_I probe failed: got 0x%02x, expected 0x%02x",
                 who_am_i[i], IIS3DWB_WHO_AM_I_VAL);
...
static esp_err_t iis3dwb_log_config_readback(iis3dwb_fs_t fs) {
  ...
  // LOG_INFOF("IIS3DWB config readback(fs=%u): CTRL1_XL=0x%02x CTRL3_C=0x%02x
  // CTRL4_C=0x%02x FIFO1=0x%02x FIFO2=0x%02x FIFO3=0x%02x FIFO4=0x%02x",
  //           (unsigned int)fs,
  //           ctrl1,
  //           ctrl3,
  //           ctrl4,
...
static esp_err_t iis3dwb_arm_fifo(void) {
  ...
  if (ret != ESP_OK)
    return ret;
  return iis3dwb_write_reg(IIS3DWB_REG_FIFO_CTRL4,
                           IIS3DWB_FIFO_MODE_CONTINUOUS);
}
...
static esp_err_t iis3dwb_log_fifo_runtime_state(void) {
  ...
  if (ret != ESP_OK)
    return ret;
  ret =
      iis3dwb_read_bytes_with_freq(IIS3DWB_REG_FIFO_DATA_OUT_TAG, fifo_sample,
                                   sizeof(fifo_sample), IIS3DWB_SPI_DEFAULT_HZ);
  if (ret != ESP_OK)
    return ret;
...
static void iis3dwb_unpack_fifo(const uint8_t *raw, imu_raw_data_t *out,
                                size_t count) {
  const uint8_t *p = raw;
  ...
  while (s_stream_running) {
    ...
    if (ret == ESP_OK) {
      if (s_stream_running && (s_data_cb || s_data_cb_ctx)) {
        ...
        imu_raw_data_t *dst =
            (raw == s_dma_ping_raw) ? s_conv_ping : s_conv_pong;
        iis3dwb_unpack_fifo(raw, dst, DMA_CHUNK_SAMPLES);
        if (!s_dma_capture_logged) {
          // LOG_INFOF("IIS3DWB DMA/FIFO capture success: first chunk %u
          // samples", DMA_CHUNK_SAMPLES);
...
esp_err_t drv_iis3dwb_start_stream(imu_data_cb_t cb) {
  ...
  if (xTaskCreatePinnedToCore(iis3dwb_dma_worker_task, "iis3dwb_dma_task", 4096,
                              NULL, configMAX_PRIORITIES - 1,
                              &s_dma_task_handle, 1) != pdPASS) {
    s_stream_running = false;
    s_data_cb = NULL;
    return ESP_ERR_NO_MEM;
  }
...
static void drv_internal_dma_callback(const imu_raw_data_t *data, size_t count,
                                      void *ctx) {
  (void)ctx;
