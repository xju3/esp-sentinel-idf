...
void app_main(void)
{
    ...
    if (is_hot_wakeup)
    {
        ...
        // LOG_INFOF("Hot wakeup detected. Valid RTC time: %ld. Starting network in background...", (long)now);
    ...
    if (g_dense_diag_remaining > 0)
    {
        ...
        if (sleep_time_us == 0 || dense_sleep_us < sleep_time_us)
        {
            ...
            // LOG_INFOF("Dense diagnostic mode active (%d times left). Overriding sleep to %d seconds...", g_dense_diag_remaining, g_dense_diag_interval_s);
        }
